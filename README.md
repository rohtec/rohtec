### Hi there 👋

<!--
*# Hello there; welcome 👋🏾

[![AltSchool Badge](https://img.shields.io/badge/-Engineering-6773E5?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&link=https://altschoolafrica.com/schools/engineering)](https://altschoolafrica.com/schools/engineering)

Hello there! I am Esther Matthew, an enthusiastic and dedicated student in the field of software engineering. With a passion for problem-solving and a knack for logical thinking, I am on an exciting journey to explore the vast realm of coding and development.

My programming journey began with using HTML & CSS to build web pages from the scratch, and since then, I have been immersed in learning the intricacies of software development. Eager to absorb knowledge and embrace challenges, I am committed to continuously enhancing my skills and staying abreast of emerging technologies.

Currently studying front end engineering at Altschool Africa, I am gaining a solid foundation in algorithms, data structures, and software design principles. My GitHub repositories showcase my evolving projects, reflecting my commitment to hands-on learning and collaborative development.

I believe in the power of technology to transform ideas into impactful solutions. As I navigate through the dynamic landscape of software engineering, I am excited to contribute my skills and collaborate with like-minded individuals to create innovative and meaningful projects.

Let's connect and code together as I embark on this thrilling adventure in the world of software engineering!.

**Here's a quick summary about me**:

- 😊 Pronouns: She/her
- 💡 Fun fact: I'm currently studying at AltSchool Africa [School of Software Engineering](https://altschoolafrica.com/schools/engineering) Class of 2023 and I'm also a student of the University of Ibadan,Nigeria.
- 🌱 I’m currently learning Semantic HTML,CSS, JavaScript, Reactjs, and Python.
- 😊 I’m looking for help with open source projects, hackathons, internships, and entry-level opportunities.
- 💼 Job interests: Software Engineer, Front Engineer, or UI Engineer (Intern or Junior level).
- 📫 You can [view my resume](#) and contact me by emailing matthewesther2019@gmail.com.

---
